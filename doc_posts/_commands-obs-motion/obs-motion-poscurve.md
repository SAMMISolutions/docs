...
Sends `{"positionX": x, "positionY": y}` as `sceneItemTransform` on every frame. The position values follow a curved arc computed from the start/end coordinates and the **Curve %** parameter.
