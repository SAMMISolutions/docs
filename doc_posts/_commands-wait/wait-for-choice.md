...
By default, only the current button is paused while the dialog is open — all other buttons continue running normally in the background. If you want all buttons to freeze until the dialog is closed (legacy behavior), enable **Wait For User Choice: Block All Buttons** in SAMMI Settings.
